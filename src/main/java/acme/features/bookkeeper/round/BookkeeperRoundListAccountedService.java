...
@Service
public class BookkeeperRoundListAccountedService implements AbstractListService<Bookkeeper, Round> {
	...
	public Collection<Round> findMany(final Request<Round> request) {
		...
		result = this.repository.findManyAccounted(request.getPrincipal().getAccountId());
